...
if [[ $? -ne 0 ]]; then
    ...
    if [ -z $CLOUD9_INSTANCE ]; then
        echo "ACTION REQUIRED: Looks like Cloud9 instance with m5large instance type is missing. Please create one!!"
...
YARN_MIN_VERSION=1.22.19
check_version $YARN_MIN_VERSION $YARN_VERSION
if [[ $? -eq 1 ]]; then
    echo "ACTION REQUIRED: Need to have Yarn [$YARN_MIN_VERSION] version greater than or equal to $YARN_MIN_VERSION"
    SUMMARY+="* ACTION REQUIRED: Need to have Yarn [$YARN_MIN_VERSION] version greater than or equal to $YARN_MIN_VERSION"$'\n'
else 
    SUMMARY+="* PASS : Yarn version $YARN_VERSION installed. The minimum required version $YARN_MIN_VERSION"$'\n'
fi
echo ""
